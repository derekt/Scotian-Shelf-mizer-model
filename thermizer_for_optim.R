...
#    # Now fill the array
   ...
   effort_array_Fhistsoc[73:dim(effort_array_Fhistsoc)[1],2] = 0.5
   ...
   ocean_temp_array_IPSL_CC585[,c(1,2,3,5,6,7,8,9)] = c(rep(bottom_temp[1,3],50), bottom_temp[1:48,3])
   ocean_temp_array_IPSL_CC585[,4] = c(rep(surface_temp[1,3],50), surface_temp[1:48,3])
...
# ```
new_Rmax = rep(1e12,length(params@species_params$R_max))
...
# #-------------------------------------
 ptm <- proc.time()
 aa = optim(rep(1e+10,9), runModel, params = params_IPSL_ssp5rcp85, t_max = length(times), effort = effort_array_Fhistsoc, control = list(parscale = rep(1e10,9)))
 proc.time() - ptm
...
bsk = params_IPSL_ssp5rcp85@other_params$other$kappa_scaling
save(bsk , file = "best_kappa_scaling.Rdata")
bsp = params_IPSL_ssp5rcp85@species_params
save( bsp , file = "best_species_params.Rdata")
