...
#this isn't working
total = merge(x,y2,by="erepro")
view(total)

#so we can do it the slow way? Idk let's try
write.csv(y2, "Desktop/y2.csv")
write.csv(x, "Desktop/x.csv")

#ok now the merged dataframe is named "joint" (saved as CSV on desktop)################################
